...
class Settings extends MX_Controller {
    ...
    function membership() {
        ...
        $data['setting_membership']  = $this->settings_model->getmembership($id);
    ...
    function addmembership(){
        ...
        $expiry_d        = $this->input->post('expiry_d');
        $membership_discount        = $this->input->post('membership_discount');
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('<div class="error">', '</div>');

        // Validating Name Field
        $this->form_validation->set_rules('m_name', 'Membership Name', 'trim|required|min_length[2]|max_length[100]|xss_clean');
         
        $this->form_validation->set_rules('membership_price', 'Membership Price', 'trim|required|min_length[2]|max_length[500]|xss_clean');
        // Validating Phone Field           
        $this->form_validation->set_rules('expiry_d', 'Expiry', 'trim|required|min_length[1]|max_length[10]|xss_clean');
        $this->form_validation->set_rules('membership_discount', 'Membership Discount', 'trim|required|min_length[1]|max_length[10]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
             $this->session->set_flashdata('feedback', 'Validation Error !');
                if (!empty($id)) {
                        ...
                        redirect("settings/membership?id=".$id);
                }else{
                ...
                $data = array(
                 ...
                 if (!empty($id)) {
                $this->settings_model->updateMembership($id,$data);
                }else{
                     $this->settings_model->insertMembership($data);
                }
    ...
    function delete_membership() {
        $id = $this->input->get('id');
        $case_history = $this->settings_model->deleteMembership($id);
