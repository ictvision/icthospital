...
//echo "<pre>";print_r($_POST);exit;
       ...
       // Validating FEE Field 
        $this->form_validation->set_rules('fee', 'Fee', 'trim|required|min_length[1]|max_length[50]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New Doctor
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $data1 = array('category' => $inserted_id.'_'.$name,
                        'description' => '',
                        'type' => 'other',
                        'c_price' => $fee,
                        'd_commission' => 0
                ...
                $this->finance_model->updatePaymentCategory($cat_id , $data1);
                    ...
                    $id_info = array('ion_user_id' => $ion_user_id,'procedure_id'=>$p_id);
                ...
                if($this->input->post('fee_id') == ''){
                     $this->finance_model->insertPaymentCategory($data1);
                     $p_id = $this->db->insert_id();
                     $id_info = array('procedure_id'=>$p_id);
                     $this->doctor_model->updateDoctor($id, $id_info);
                }else{
                    $this->finance_model->updatePaymentCategory($this->input->post('fee_id') , $data1);
                }
    ...
    function editDoctorByJason() {
        $id                 = $this->input->get('id');
         ...
         $data['doctor']     = $this->doctor_model->getDoctorById($id);
        if( $data['doctor']->procedure_id !=NULL || $data['doctor']->procedure_id!='' ){
            $data['procedure']  = $this->finance_model->getPaymentCategoryById($data['doctor']->procedure_id);
        }else{
