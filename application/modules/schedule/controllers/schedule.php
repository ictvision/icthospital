...
class Schedule extends MX_Controller {
    ...
    function addSchedule() {
        ...
        $get      = $this->input->get('doctor');
       // echo "<pre>".$get ;print_r($this->input->post());exit;
        if (empty($redirect)) {
        ...
        if (empty($id)) {
            ...
            if (!empty($check)) {
                ...
                redirect($redirect.'/timeSchedule?doctor='.$doctor);
                die();
            }
        }
...
// Validating Week Day Field   
    ...
    function getAvailableSlotByDoctorByDateByJason() {
        ...
        $doctr     = $this->doctor_model->getDoctorById($doctor);
        if( $doctr->procedure_id != NULL || $doctr->procedure_id !='' ){

            $data['procedure']   = $this->finance_model->getPaymentCategoryById($doctr->procedure_id);
        }else{
