...
class Lab extends MX_Controller {
    ...
    public function index() {
        ...
        $data['lab_centers']  = $this->lab_model->getLabCenter();
            ...
            $this->upload->initialize($config);
             if ($this->upload->do_upload('img_url')) {
               ...
               // echo $img_url;
                 ...
                 $this->session->set_flashdata('feedback', 'image type should be png,jpg,gif or jpeg');
            //echo "heloo";
...
//echo   $img_url;exit;
        ...
        $picker_name  = $this->input->post('picker_name');
       ...
       // exit;
        ...
        $this->form_validation->set_rules('discount', 'Discount', 'trim|min_length[1]|max_length[100]|xss_clean');
         ...
         //$this->form_validation->set_rules('img_url', 'Upload pic', 'trim|required|min_length[1]|max_length[100]|xss_clean');
        ...
        $this->form_validation->set_rules('sample_no', 'Sample No', 'required');
        $this->form_validation->set_rules('picker_name', 'Picker Name', 'required');
                ...
                $temp_emp = implode('',$rep_tem);
                 ...
                 if (empty($id)) {
                        ...
                        $data = array(
                            ...
                            'lab_center_id'       => $center_id ,
                            'sample_picker_name'  => $picker_name ,
...
// Adding New Patient
    ...
    public function addTemplate() {
        ...
        $sample_required        = $this->input->post('sample_required');
        $reporting_day        = $this->input->post('reporting_day');
        $description = '';
        ...
        // Validating Price Field
        $this->form_validation->set_rules('name', 'Template', 'trim|min_length[1]|max_length[100]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
           if(empty($id)){
            ...
            redirect('lab/editTemplate?id='.$id);
        } else {
    ...
    function deleteTemplate() {
        ...
        $temp = $this->lab_model->getTemplateById($id);
       // $data['template']  = $this->lab_model->getTemplateById($id);
        // $cat_name         = $data['template']->name;
        $this->finance_model->deletePaymentCategory($temp->procedure_id);
    ...
    function getLab() {
        ...
        foreach ($data['labs'] as $lab) {
            ...
            if (!empty($patient_info)) {
                ...
                $patient_details = $patient_info->name;
                $patient_details_phone = $patient_info->phone;
            } else {
                $patient_details = ' ';
                $patient_details_phone = ' ';
            }
            if(empty($lab->status)){
                $st = 'in-process';
            }else{
