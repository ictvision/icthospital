...
class Patient extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->library('includes/BigBlueButton');
    ...
    public function index() {
        ...
        $data['setting_memberships'] = $this->settings_model->getmemberships();
        //echo "<pre>";print_r( $data['setting_memberships']);exit;
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('patient', $data);
        $this->load->view('home/footer'); // just the header file
    }

    public function join_meetings(){

        if ($this->ion_auth->in_group(array('Doctor'))) {
            $patient_ion_id = $this->ion_auth->get_user_id();
            $patient_d = $this->db->get_where('patient', array('ion_user_id' => $patient_ion_id))->row()->id;
            $data['appointments'] = $this->patient_model->getmettings($patient_d);
        ...
        $data['doctors']  = $this->doctor_model->getDoctor();
        $data['settings'] = $this->settings_model->getSettings();
//echo "<pre>";print_r($data['appointments'] );
         $this->load->view('home/dashboard', $data); // just the header file
        $this->load->view('p_meetings', $data);
        //echo "adee;";exit;
        $this->load->view('home/footer'); // just the header file
    }

    public function meeting(){
        $meeting_id = $this->input->get('id');
       ...
       $m_data =  $this->patient_model->getmettingid($meeting_id);

      // echo "<pre>";print_r( $m_data);exit;

        $joinParams = array(
            'meetingId' =>$meeting_id,          // REQUIRED - We have to know which meeting to join.
            'username' => $m_data->attendeePw,   // REQUIRED - The user display name that will show in the BBB meeting.
            'password' =>$m_data->attendeePw,             // REQUIRED - Must match either attendee or moderator pass for meeting.
        ...
        if ($itsAllGood == true) {
            ...
            <script type="text/javascript">
                window.open("<?php  echo $result; ?>", '_blank');
                window.history.back();
            </script>
    ...
    function getmetings() {
        ...
        foreach ($data['appointments'] as $patient) {
            ...
            if ($this->ion_auth->in_group(array('admin', 'Accountant', 'Receptionist', 'Laboratorist', 'Nurse', 'Doctor'))) {
                ...
                $options1 = ' <a type="button" class="btn editbutton" title="' . lang('edit') . '" data-toggle = "modal" data-id="' . $patient->id . '"><i class="fa fa-edit"> </i> ' . 'Join'. '</a>';
            }
...
*/
    ...
    public function indoor() {
        ...
        //echo "<pre>";print_r( $data['setting_memberships']);exit;
        $this->load->view('home/dashboard'); // just the header file
        $this->load->view('indoor_patient', $data);
        $this->load->view('home/footer'); // just the header file
    }
    public function mr_registration(){

        $patients = $this->patient_model->getindoorPatient();
        $total = count($patients) + 1;
        $patient_id = date("Y")."_".$total ;
    ...
    public function membership(){
        ...
        $patient_user_id = $this->db->get_where('patient', array('id' => $id ))->row()->patient_id;
        $data['membership_details'] =$this->patient_model->getMembershipdetails($patient_user_id);
        $data['setting_memberships'] = $this->settings_model->getmemberships();
        $patient        = $id;
        $data['expiry'] = check_expiry($id);
        //echo check_expiry($id);
        //echo "<pre>";print_r( $data['expiry']);
        //exit;
        ...
        //echo "<pre>".$patient;print_r($data['patient']);exit;
    ...
    public function addNew() {
        ...
        if(!isset($_POST['outdoor']) && $_POST['outdoor']!='outdoor'){
            $type='indoor';
         $this->form_validation->set_rules('m_r', 'Medical Registration Number', 'required|trim|xss_clean');
         $this->form_validation->set_rules('remark', 'Remark', 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New Patient
                if ($this->ion_auth->email_check($email)) {
                    ...
                    if (!empty($sms)) {
                        ...
                        if($this->input->post('membership')!=''){
                         ...
                         $card_no            = random_string('numeric', 14);
                         $setting_membership = $this->settings_model->getmembership($this->input->post('membership'));
                         $cdate              =  date("Y-m-d H:i:s");
                         $expery_date        =  date("Y-m-d H:i:s", strtotime("+".$setting_membership->expiry." month", strtotime($cdate))); //2015-05-22 10:35:10
                         //if()
                         $membership_data = array(

                                                 'patient_id'      => $patient_id,
                                                 'membership_type' => $this->input->post('membership'),
                                                 'card_no'         => $card_no,
                                                 'date'            => $cdate,
                                                 'expiry'          => $expery_date,
                                                 'status'          =>'Active',
                        ...
                        $this->patient_model->insertPatientMembership($membership_data);
                ...
                if($this->input->post('membership')!=''){
                         ...
                         $card_no           = random_string('numeric', 14);
                         $cdate             =  date("Y-m-d H:i:s");
                         $expery_date       =  date("Y-m-d H:i:s", strtotime("+1 years", strtotime($cdate))); //2015-05-22 10:35:10
                         $dbexpery_date     =  date("Y-m-d H:i:s", strtotime("+1 years", strtotime($cdate))); //2015-05-22 10:35:10
                         
                         if($cdate>=$dbexpery_date){
                         $membership_data = array(
    ...
    function membership_add(){
        ...
        // echo "<pre>";print_r($membership_data);
         //exit;
        $this->patient_model->insertPatientMembership($membership_data);
            //patient/membership?id=35
            redirect('patient/membership?id='.$id );
    }
    ...
    function documents() {
        $data['patients'] = $this->patient_model->getindoorPatient();
...
// Validating Deposited Amount Field
    ...
    function getPatient() {
        ...
        foreach ($data['patients'] as $patient) {
            ...
            $options2 = '<a class="btn detailsbutton" title="' . lang('info') . '" style="color: #fff;" href="patient/patientDetails?id=' . $patient->id . '"><i class="fa fa-info"></i> ' . lang('info') . '</a>';
            $options7 = '<a class="btn detailsbutton" title="Membership' . lang('membership') . '" style="color: #fff;" href="patient/membership?id=' . $patient->id . '"><i class="fa address-card"></i> Membership' . lang('membership') . '</a>';
            ...
            if ($this->ion_auth->in_group(array('admin'))) {
                $info[] = array(
                    ...
                    $patient->phone,
                    $this->settings_model->getSettings()->currency . $this->patient_model->getDueBalanceByPatientId($patient->id),
                    $options1 . ' ' . $options6 . ' ' . $options3 . ' ' . $options4 . ' ' .$options7.' '. $options5,
                        //  $options2
                );
            }
            ...
            if ($this->ion_auth->in_group(array('Accountant', 'Receptionist'))) {
                $info[] = array(
                    ...
                    $patient->phone,
                    $this->settings_model->getSettings()->currency . $this->patient_model->getDueBalanceByPatientId($patient->id),
                    $options1 . ' ' . $options6 . ' ' .$options7. ' ' . $options4,
                        //  $options2
                );
            }
            ...
            if ($this->ion_auth->in_group(array('Laboratorist', 'Nurse', 'Doctor'))) {
                $info[] = array(
                    ...
                    $patient->name,
                    $patient->phone,
                    $options1 . ' ' . $options6. ' ' .$options7 . ' ' . $options3,
                        //  $options2
                );
            }
        }
        ...
        if (!empty($data['patients'])) {
            $output = array(
                "draw" => intval($requestData['draw']),
                "recordsTotal" => $this->db->get_where('patient', array('type' => 'outdoor'))->num_rows(),
                "recordsFiltered" => $this->db->get_where('patient', array('type' => 'outdoor'))->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
    ...
    function getPatientindoor() {
        ...
        if ($limit == -1) {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getPatientIndorBysearch($search);
            } else {
                $data['patients'] = $this->patient_model->getPatientIndor();
            }
        } else {
            if (!empty($search)) {
                $data['patients'] = $this->patient_model->getPatientIndorByLimitBySearch($limit, $start, $search);
            } else {
                $data['patients'] = $this->patient_model->getPatientIndorByLimit($limit, $start);
            }
        }
        //  $data['patients'] = $this->patient_model->getPatient();
        ...
        if (!empty($data['patients'])) {
            $output = array(
                "draw" => intval($requestData['draw']),
                "recordsTotal" => $this->db->get_where('patient', array('type' => 'indoor'))->num_rows(),
                "recordsFiltered" => $this->db->get_where('patient', array('type' => 'indoor'))->num_rows(),
                "data" => $info
            );
        } else {
            $output = array(
