...
class Appointment extends MX_Controller {
    ...
    $this->meetting_create();

}

 public function meetting_create(){
    ...
    if ($this->ion_auth->in_group(array('Doctor'))) {
            ...
            $data['appointments'] = $this->appointment_model->getAppointmentById($this->input->get('id'));
        }
            $patient_id     = $data['appointments']->patient;
            $doc_id         = $data['appointments']->doctor;
            $patientdata    =  $this->db->get_where('patient', array('id' => $patient_id ))->row()/*->name*/;

        if (!empty($doc_id)) {
            $docdata =  $this->db->get_where('doctor', array('id' => $doc_id))->row()/*->name*/;
        }
        ...
        */
        ...
        $creationParams = array(
            'meetingId' => $this->input->get('id').$doc_id.$patient_id ,                  // REQUIRED
            'meetingName' => 'Tele health',   // REQUIRED
            'attendeePw' => $patientdata->name,                   // Match this value in getJoinMeetingURL() to join as attendee.
            'moderatorPw' => $docdata->name,                  // Match this value in getJoinMeetingURL() to join as moderator.
    ...
    if ($itsAllGood == true) {
        ...
        if ($result == null) {
            ...
            if ($result['returncode'] == 'SUCCESS') {
                 ...
                 $check_m = $this->meeting_model->getMeetingid($result['meetingId']);
                    /*echo "<pre>";print_r( $check_m );
                    exit;*/
                if(!$check_m){
                    //exit;
                 $metind_data = array(
                    'appointment_id' =>$this->input->get('id'),                     // ''= use default. Change to customize.
                    'p_id' =>$patient_id,                     // ''= use default. Change to customize.
                    'dr_id' => $doc_id,                     // The main number to call into. Optional.
                    'meeting_id' => $result['meetingId'],                    // PIN to join voice. Optional.
                    'attendeePw' => $patientdata->name,                       // Alphanumeric to join voice. Optional.
                    'deratorPw' =>  $docdata->name,                      // Default in          
                );

                 $this->meeting_model->insertMeeting($metind_data);
                ...
                $this->join_meetting($result['meetingId'],$patientdata->name,$docdata->name);
//print_r($result);
 ...
 public function join_meetting($meeting_id,$atendee_pass,$moderatorPw){
    ...
    */
    ...
    $joinParams = array(
        ...
        'username' => $moderatorPw,      // REQUIRED - The user display name that will show in the BBB meeting.
    ...
    try {
       ...
       // exit;
        ...
        ?>
            <!-- <iframe src="<?php echo $result; ?>" title="">
            </iframe>  -->
           ...
           // redirect($_SERVER['HTTP_REFERER']);
               ...
               /* $data['result'] =  $result;
                ...
                $data['attendee'] = $atendee_pass;
                $data['moderator'] =$moderatorPw ;
    ...
    public function addNew() {
       ...
       // exit;
        ...
        $this->form_validation->set_rules('doc_fee', 'Remarks', 'trim|min_length[1]|max_length[1000]|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New department
                $appointment = $this->appointment_model->insertAppointment($data);
                ...
                $current_item   = $this->finance_model->getPaymentCategoryById($this->input->post('p_id'));
                $category_price = $current_item->c_price;
                $category_type  = $current_item->type;
                $qty = 1;
                $cat_and_price[] = $this->input->post('p_id'). '*' . $category_price . '*' . $category_type . '*' . $qty;
                ...
                $category_name = implode(',', $cat_and_price);
                    ...
                    $d_commission = $this->finance_model->getPaymentCategoryById($this->input->post('p_id'))->d_commission;
                   
                    $h_commission = 100 - $d_commission;
                    $hospital_amount_per_unit =  1 * $h_commission / 100;
                    $hospital_amount = $hospital_amount_per_unit * $qty;
               // }
                ...
                $get_cat = $this->finance_model->getPaymentCategoryById($this->input->post('p_id'));
                $patient_details = $this->patient_model->getPatientById($patient);
                $doctor_detail = $this->doctor_model->getDoctorById($doctor);
                //$user = $this->ion_auth->user();
                $data = array(
                    'category_name'   => $category_name,//$get_cat->category,
                    'patient'         => $patient,
                    'date'            => time(),
                    'amount'          => $sub_total,
                    'doctor'          => $doctor,
                    ...
                    //'amount_received' => $hospital_amount,
            ...
            // Loading View
            if($status=='Confirmed' && $appointment_id!=''){
                //$print="?print=yes&appointment_id=".$appointment_id.'&payment_id='.$inserted_id;
                $print="?id=".$inserted_id."&print=yes&appointment_id=".$appointment_id.'&payment_id='.$inserted_id;
            }else{
                $print="";
            }
            if (!empty($redirect)) {
                redirect($redirect.$print);
            } else {
                redirect('appointment'.$print);
            }
        }
    }
    ...
    function printtoken(){
      ...
      $appointment_id = $this->input->get('appointment_id');
         ...
         $data[] = $this->appointment_model->getAppointmentById($appointment_id);
        //echo "<pre>";print_r($data);

          $date = date('Y-m-d', $data[0]->date);
            $doctor = $this->doctor_model->getDoctorById($data[0]->doctor);
            $patient_details = $this->patient_model->getPatientById($data[0]->patient);
           //echo "<pre>";print_r($doctor);
            $data2[] = $this->schedule_model->getallAvailableSlotByDoctorByDateByAppointmentId($date, $data[0]->doctor, '');
            ...
            // exit;
            $i=1;
            foreach($data2[0] as $slot){
                //echo $slot;
                if($data[0]->time_slot==$slot){
                    $token = $i;
                }
                $i++;
            }
            //echo $token;
            //echo "<pre>dd";print_r($data2);

            $print_data = array();

            $print_data['token']    = $token;
            $print_data['Doctor']   = $doctor->name;
            $print_data['patient']  = $patient_details->name;
            $print_data['settings'] = $this->settings_model->getSettings();
             //echo "<pre>";print_r($print_data);
            //$this->load->view('home/dashboard', $print_data); // just the header file
